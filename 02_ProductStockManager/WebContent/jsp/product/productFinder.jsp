...
<%
	...
	try{
		...
		throw new ProductException("상품검색오류!", e);
	}
...
$(function(){
	...
	$("table tr").click(function(){
		...
		location.href 
		= "<%=request.getContextPath()%>/jsp/product/productView.jsp?productId="+productId;
	...
	$("[name=srchType]").change(function(){
		var id = $(this).attr("id");
		if(id === 'productId')
			$("#srchKeyword").attr("placeholder","검색할 상품 아이디를 입력하세요.")
							 .focus();
		else
...
<div id="container">
	...
	<section id="srch-container">
		<form name="srchFrm" 
			...
			<input type="radio" name="srchType" id="productId" value="productId" 
					<%="productId".equals(srchType)?"checked":"" %>/>
			<label for="productId">상품아이디</label>
			<input type="radio" name="srchType" id="productName" value="productName" 
					<%="productName".equals(srchType)?"checked":"" %>/>
